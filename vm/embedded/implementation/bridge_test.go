...
func TestChangeTssECDSAPubKey_ValidateSendBlock(t *testing.T) {
	...
	// Correct length, but does not resolve to a point on the curve.
...
// A key that does not resolve must be reported as an error rather than passed on.
func TestChangeTssECDSAPubKey_OffCurveKeyIsRejected(t *testing.T) {
	method := &ChangeTssECDSAPubKeyMethod{definition.ChangeTssECDSAPubKeyMethodName}
	offCurve := base64.StdEncoding.EncodeToString(make([]byte, constants.CompressedECDSAPubKeyLength))

	defer func() {
		if r := recover(); r != nil {
			t.Fatalf("unresolvable public key did not return an error cleanly: %v", r)
		}
	}()
