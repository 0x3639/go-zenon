...
const (
	...
	// Total derivations exposed in the dev table. 0/4/6/10 are producer keys
	// (live in pillar containers); 1/5/7/11 are pillar owners (held by the
	// user); 2 is spork; 3/8/9 are funded general dev accounts.
	derivationCount uint32 = 12
)

// pillarSpec describes one of the devnet pillars. The producer key for
...
var pillars = []pillarSpec{
	...
	{Role: "pillar4", Dir: filepath.Join(devnetDir, "pillar4"), IP: "172.30.0.15", ProducerIndex: 10, OwnerIndex: 11},
}
