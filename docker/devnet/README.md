...
| Service    | Static IP       | Role                                           | Host ports        |
|------------|-----------------|------------------------------------------------|-------------------|
| `pillar`   | `172.30.0.10`   | Pillar 1 producer + bootstrap (`MinPeers=0`)   | `35991` HTTP RPC  |
| `pillar2`  | `172.30.0.12`   | Pillar 2 producer                              | _none exposed_    |
| `pillar3`  | `172.30.0.13`   | Pillar 3 producer                              | _none exposed_    |
| `rpc`      | `172.30.0.11`   | Public RPC ingress                             | `35997`, `35998`  |
| `observer` | `172.30.0.14`   | Non-pillar observer / relay peer               | _none exposed_    |
| `explorer` | Docker-assigned | Static Zenon explorer                          | `35999`           |

Container names are generated by Docker Compose from the project and service
name. Prefer `docker compose exec <service> ...` over hardcoded container names.

The node services share the bridge network `znnd-devnet` (`172.30.0.0/24`).
...
Image names and host ports can be overridden with Compose environment
variables; see [`.env.example`](../../.env.example) for the defaults.
...
```sh
curl -s http://localhost:35999/devnet-endpoint.js
...
To poke them directly use `docker compose exec`:

```sh
docker compose exec pillar2 wget -qO- \
